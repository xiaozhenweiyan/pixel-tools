...
- **用途**：物理沙盒模拟器，类似 Falling Sand Game。支持沙子、水、石头、火、植物、金属等元素，元素间真实互动（火燃烧植物、金属被火加热变红、植物遇水生长等）。
- **关键函数**：`initPhysicsSandbox()`、`step()`（每帧更新网格）、`paintCell(x, y, element)`、`interactCells()`、`setBrushSize(n)`。
- **依赖**：Canvas 2D API、`js/i18n.js`。
...
#### `js/pixel-rpg.js`
- **用途**：像素 RPG 地牢迷宫探险小游戏。戴面具黑衣人闯关，回合制战斗，墙上火把照亮前路，史莱姆为主要怪物，向下走廊通往下一层。
- **关键函数**：`initRPG()`、`generateDungeon(level)`、`handlePlayerMove(dx, dy)`、`startBattle(enemy)`、`takeTurn(action)`、`nextFloor()`。
...
不会。本项目零后端依赖，所有计算、存储、渲染都在浏览器中完成。图像像素化、绘图导出全部在客户端处理，图片不会上传到任何服务器。用户数据仅保存在 `localStorage` / `IndexedDB` 中，关闭浏览器即销毁（除非用户主动保留）。
